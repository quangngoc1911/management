...
namespace ManagementSystem.Modules.Menus.Application.Services;
...
public class MenuService : IMenuService
{
    ...
    public async Task<MenuDto> CreateAsync(CreateMenuDto dto)
    {
        if (dto.ParentId.HasValue)
        {
            ...
            if (parent == null)
                throw new KeyNotFoundException($"KhĂ´ng tĂ¬m tháº¥y menu cha Id={dto.ParentId}");
        }

        if (string.IsNullOrWhiteSpace(dto.Name))
            throw new ArgumentException("TĂªn menu khĂ´ng Ä‘Æ°á»£c Ä‘á»ƒ trá»‘ng");
    ...
    public async Task<MenuDto?> UpdateAsync(Guid id, CreateMenuDto dto)
    {
        ...
        if (dto.ParentId.HasValue && dto.ParentId.Value != menu.ParentId)
        {
            if (dto.ParentId.Value == id)
                throw new ArgumentException("Menu khĂ´ng thá»ƒ lĂ  cha cá»§a chĂ­nh nĂ³");
